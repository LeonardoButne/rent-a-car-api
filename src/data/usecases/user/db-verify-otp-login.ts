...
import { VerifyOtpLoginForClient } from '../../../domain/usecases/user-usecases/verify-otp-login-client-usecase'
...
export class DbVerifyOtpLogin implements VerifyOtpLoginForClient {
    ...
    async verify(otp: string, email: string): Promise<string | boolean> {
        const account =
            await this.getAccountAdminByEmailRepository.getAccountByEmail(email)
