...
import { VerifyOtpLoginUserUsecase } from '../../domain/usecases/verify-otp-login-user-usecase';

export class VerifyOtpLoginUserController implements Controller {
  constructor(
    private readonly validation: Validation,
    private readonly verifyOtpLoginUser: VerifyOtpLoginUserUsecase,
  ) {}
  async handle(httpRequest: HttpRequest): Promise<HttpResponse> {
    try {
      ...
      // }
      const token = await this.verifyOtpLoginUser.verify({ otp, email, deviceId });
      if (token === false) {
